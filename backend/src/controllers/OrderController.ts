...
export class OrderController {
  async initiateCheckout(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const order = await orderService.initiateCheckout(userId);
      res
        .status(201)
        .json({ message: "Order created and stock reserved.", order });
    } catch (error) {
  ...
  async payOrder(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const order = await orderService.payOrder(orderId, userId);
